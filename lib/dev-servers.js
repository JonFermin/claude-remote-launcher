...
export async function handleKillDevServer(res, json, port, PORT) {
  // Prevent killing the launcher's own server
  if (port === PORT) {
    return json(res, 403, { error: "Cannot kill the launcher server" });
  }
  ...
  try {
    if (process.platform === "win32") {
      // Find PIDs listening on this port, then kill them
      const out = execSync(
        `powershell -NoProfile -Command "(Get-NetTCPConnection -LocalPort ${port} -State Listen -ErrorAction SilentlyContinue).OwningProcess"`,
        { timeout: 5000 }
      ).toString().trim();
      const pids = [...new Set(out.split(/\r?\n/).map(Number).filter(Boolean))];
      if (!pids.length) return json(res, 404, { error: "No process found on port " + port });
      for (const pid of pids) {
        execSync(`taskkill /PID ${pid} /T /F`, { timeout: 5000, stdio: "ignore" });
      }
    } else {
      // macOS / Linux: use lsof to find PID
      const out = execSync(`lsof -ti :${port} -sTCP:LISTEN`, { timeout: 5000 }).toString().trim();
    ...
    // Clear cache so next refresh picks up the change
    lastDevServers = lastDevServers.filter(s => s.port !== port);
    json(res, 200, { killed: port });
  } catch (err) {
    json(res, 500, { error: "Failed to kill process on port " + port, detail: err.message });
  }
}
