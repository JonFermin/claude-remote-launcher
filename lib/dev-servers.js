...
// Cache the PowerShell port scan for a few seconds so the UI's 5s polling
// doesn't re-exec powershell on every tick (~100–300ms per call).
const PORT_SCAN_TTL_MS = 4000;
let portScanCache = null; // { ports, expiresAt }
...
export function identifyServer(probe) {
...
async function scanListeningPorts(PORT) {
  const now = Date.now();
  if (portScanCache && now < portScanCache.expiresAt) return portScanCache.ports;
  // Coalesce concurrent callers onto a single powershell invocation.
  if (portScanInflight) return portScanInflight;

  portScanInflight = (async () => {
    let ports = [];
    try {
      ...
      ports = stdout.trim().split(/\r?\n/).map(Number).filter((p) => p && p !== PORT);
    } catch { /* fallback: empty */ }
    portScanCache = { ports, expiresAt: Date.now() + PORT_SCAN_TTL_MS };
    return ports;
  })().finally(() => { portScanInflight = null; });
...
export async function handleDevServers(res, json, PORT, TAILNET_DOMAIN) {
  ...
  const ports = await scanListeningPorts(PORT);
