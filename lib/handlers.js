...
import { sessions, persistSessions, isProcessAlive, checkSessionHealth, createRingBuffer } from "./sessions.js";
  ...
  function handleDelete(res, id) {
    ...
    if (s.proc) {
      if (process.platform === "win32") {
        spawn("taskkill", ["/PID", String(s.proc.pid), "/T", "/F"], { stdio: "ignore" });
      } else {
        s.proc.kill("SIGTERM");
      }
    }
