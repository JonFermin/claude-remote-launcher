...
// ---------------------------------------------------------------------------
export function createHandlers({ json, MAX_SESSIONS, ALLOWED_DIRS, VALID_PERMISSION_MODES, VALID_SPAWN_MODES, NAME_RE, isAllowedDir, CLAUDE_BIN }) {
  ...
  function handleStatus(res, id) {
    ...
    const { proc, stdout, stderr, ...safe } = s;
    const stdoutStr = stdout?.toString?.() ?? "";
    json(res, 200, {
      ...safe,
      alive: isProcessAlive(s.proc),
      stdout: stdoutStr.slice(-2000),
    });
  }
  ...
  function spawnRemoteControl(session, args, workDir) {
    ...
    // Preserve buffers across retries so diagnostic output from the failed
    // run isn't lost. First-time init happens in handleRemoteControl.
    if (!session.stdout) session.stdout = createRingBuffer();
    if (!session.stderr) session.stderr = createRingBuffer();
    if (session._retries > 0) {
      session.stdout.append(`\n--- retry ${session._retries} ---\n`);
    }
    session.lastActivityAt = new Date().toISOString();

    // NOTE: Status detection below depends on the wording of `claude
    // remote-control --verbose` output. If those strings change upstream,
    // sessions will get stuck in "connecting" — revisit these regexes.
  ...
  function handleProjects(res) {
    ...
    // Prepend one root entry per allowed dir, labelled with the dir name so
    // multiple roots don't collide under a single "(Root)" label.
    for (const dir of [...ALLOWED_DIRS].reverse()) {
      const normalized = dir.replace(/\\/g, "/").replace(/\/$/, "");
      const label = normalized.split("/").pop() || normalized;
      projects.unshift({ name: `${label} (Root)`, path: normalized });
    }
