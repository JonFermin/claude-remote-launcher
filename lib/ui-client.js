...
// Escape strings for use inside JS string literals within HTML onclick attributes.
// HTML entity encoding alone is insufficient — the HTML parser decodes entities
// before passing to the JS engine, so we must JS-escape first, then HTML-escape.
function escapeJsStringInAttr(str) {
  return escapeHtml(String(str).replace(/\\/g, '\\\\').replace(/'/g, "\\'").replace(/"/g, '\\"'));
}
...
function renderRecentLaunches() {
  ...
  container.innerHTML = recent.map(r => {
    const safeCwd = escapeJsStringInAttr(r.cwd);
    const safePermMode = escapeJsStringInAttr(r.permMode);
...
function renderSessionList(sessions) {
  ...
  container.innerHTML = '<div class="card entries">' +
    sorted.map(s => {
      ...
      const safeCwd = escapeJsStringInAttr(s.cwd || '');
      const safePermMode = escapeJsStringInAttr(s.permMode || 'bypassPermissions');
      const safeId = escapeJsStringInAttr(s.id);
      const safeDirName = escapeJsStringInAttr(dirName);
...
function renderDevServers(servers) {
  ...
  container.innerHTML = servers.map(s => {
    ...
    // Sanitize URLs: only allow http/https/exp protocols to prevent javascript: XSS
    const urlProtocol = (s.url || '').split(':')[0].toLowerCase();
    const sanitizedUrl = ['http', 'https'].includes(urlProtocol) ? s.url : '';
    const safeUrl = escapeAttr(sanitizedUrl);
    const safeUrlJs = escapeJsStringInAttr(sanitizedUrl);
    const expoProtocol = (s.expoUrl || '').split(':')[0].toLowerCase();
    const sanitizedExpoUrl = ['http', 'https', 'exp'].includes(expoProtocol) ? s.expoUrl : '';
    const safeExpoUrl = sanitizedExpoUrl ? escapeAttr(sanitizedExpoUrl) : '';
    const expoBtn = safeExpoUrl ? `<a class="btn-primary" href="${safeExpoUrl}" target="_blank" rel="noopener noreferrer" style="background:#AE81FF;color:#1a1a1a">Expo Go</a>` : '';
    ...
    return `<div class="dev-card">
      ...
      <div class="dev-card-actions">
        <button class="btn-ghost" onclick="copyToClipboard('${safeUrlJs}')">${ICONS.clipboard} Copy URL</button>
