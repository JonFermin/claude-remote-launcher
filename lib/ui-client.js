...
function showConfirm(title, message, onConfirm) {
  ...
  overlay.innerHTML = `<div class="modal">
    ...
    <div class="modal-actions">
      <button id="modal-cancel" style="background:var(--color-border);color:var(--color-text)">Cancel</button>
      <button id="modal-confirm" style="background:var(--color-danger);color:var(--color-text)">Kill</button>
    </div>
  </div>`;
...
function renderRecentLaunches() {
  ...
  container.innerHTML = recent.map(r =>
    `<div class="recent-card" onclick="relaunchSession('${r.cwd.replace(/\\/g, '\\\\')}','${r.permMode}')">
      <div>
        <div class="recent-name">${r.name}</div>
        <div class="recent-meta">${r.permMode} \u00b7 ${timeAgo(r.ts)}</div>
      </div>
...
function renderSessionList(sessions) {
  ...
  container.innerHTML = '<div class="card entries">' +
    sorted.map(s => {
      ...
      const escapedCwd = s.cwd ? s.cwd.replace(/\\/g, '\\\\') : '';
      const buttons = stopped
        ? `<button onclick="relaunchSession('${escapedCwd}','${s.permMode || 'bypassPermissions'}')">Relaunch</button><button onclick="killSession('${s.id}','${dirName}')" style="background:var(--color-danger);color:var(--color-text)">Kill</button>`
        : `<button onclick="killSession('${s.id}','${dirName}')" style="background:var(--color-danger);color:var(--color-text)">Kill</button>`;
      ...
      return `<div class="entry${inactive}">
        <div style="min-width:0;flex:1">${spinnerHtml}<b>${dirName}</b> <span class="tag ${s.status}">${s.status}</span><br><span style="color:var(--color-muted);font-size:0.75rem"><span class="mono">${s.permMode || '\u2014'}</span>${ago ? ' \u00b7 ' + ago : ''}${uptime}${connHtml}</span></div>
...
function renderDevServers(servers) {
  ...
  container.innerHTML = servers.map(s => {
    ...
    const expoBtn = s.expoUrl ? `<a class="btn-primary" href="${s.expoUrl}" target="_blank" rel="noopener noreferrer" style="background:#AE81FF;color:#1a1a1a">Expo Go</a>` : '';
    ...
    const hostHint = cachedTailnetDomain
      ? `".${cachedTailnetDomain}"`
      : `"${s.url.replace('http://', '').split(':')[0]}"`;
    const hostWarn = s.needsHostAllow
      ? `<div class="dev-card-warn">Host blocked \u2014 add <span class="mono">allowedHosts: [${hostHint}]</span> to vite.config server options, or run <span class="mono">tailscale serve --bg ${s.port}/${s.port}</span></div>`
      : '';
    return `<div class="dev-card">
      <div class="dev-card-header">
        <span class="health-dot ${s.needsHostAllow ? 'error' : 'healthy'}"></span>
        <span class="tag" style="background:${color};color:var(--color-bg)">${s.name}</span>
        <span class="dev-card-port">:${s.port}</span>
        ${servedBadge}
      </div>
      <div class="dev-card-url">${s.url}</div>
      ${hostWarn}
      <div class="dev-card-actions">
        <button class="btn-ghost" onclick="copyToClipboard('${s.url}')">${ICONS.clipboard} Copy URL</button>
        <a class="btn-primary" href="${s.url}" target="_blank" rel="noopener noreferrer">${ICONS.external} Open</a>
        ${expoBtn}
      </div>
    </div>`;
  }).join('');
...
function killSession(id, name) {
  showConfirm('Kill session?', 'Kill session <b>' + (name || id) + '</b>?', async () => {
