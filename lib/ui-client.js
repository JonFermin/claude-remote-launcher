...
function renderRecentLaunches() {
  ...
  container.innerHTML = recent.map(r => {
    const safeCwd = escapeAttr(r.cwd);
    const safePermMode = escapeAttr(r.permMode);
    return `<div class="recent-card" data-action="relaunch" data-cwd="${safeCwd}" data-perm="${safePermMode}">
      <div>
...
function renderSessionList(sessions) {
  ...
  container.innerHTML = '<div class="card entries">' +
    sorted.map(s => {
      ...
      const safeCwd = escapeAttr(s.cwd || '');
      ...
      const buttons = stopped
        ? `<button data-action="relaunch" data-cwd="${safeCwd}" data-perm="${safePermMode}">Relaunch</button><button data-action="kill" data-id="${safeId}" data-name="${safeDirName}" style="background:var(--color-danger);color:var(--color-text)">Kill</button>`
        : `<button data-action="kill" data-id="${safeId}" data-name="${safeDirName}" style="background:var(--color-danger);color:var(--color-text)">Kill</button>`;
  ...
  container.innerHTML = servers.map(s => {
    ...
    return `<div class="dev-card">
      ...
      <div class="dev-card-actions">
        <button class="btn-ghost" data-action="copy-url" data-url="${safeUrl}">${ICONS.clipboard} Copy URL</button>
...
function renderTokenSetup() {
  ...
  app.innerHTML = `
    <div id="token-setup">
      ...
      <button data-action="save-token">Connect</button>
    </div>`;
}
...
async function renderMain() {
  ...
  app.innerHTML = `
    <div class="topbar">
      ...
      <button class="topbar-btn" data-action="toggle-settings">${ICONS.settings}</button>
    </div>

    <div class="settings-overlay" id="settings-overlay" data-action="settings-backdrop">
    ...
    <div class="tab-content">
      <div id="page-sessions" class="tab-page active">
        <div class="section-header">
          ...
          <button id="refresh-sessions" class="refresh-btn" data-action="refresh-sessions">${ICONS.refresh}</button>
        </div>
      ...
      <div id="page-launch" class="tab-page">
        ...
        <div class="card">
          ...
          <button id="launch-btn" data-action="launch">Launch Remote Control</button>
        </div>
      ...
      <div id="page-servers" class="tab-page">
        <div class="section-header">
          ...
          <button id="refresh-servers" class="refresh-btn" data-action="refresh-servers">${ICONS.refresh}</button>
        </div>
    ...
    <div class="tabbar">
      <button id="tab-sessions" class="tab active" data-action="switch-tab" data-tab="sessions">
        ${ICONS_FILLED.sessions}
        <span>Sessions</span>
      </button>
      <button id="tab-launch" class="tab" data-action="switch-tab" data-tab="launch">
        ${ICONS_OUTLINE.launch}
        <span>New</span>
      </button>
      <button id="tab-servers" class="tab" data-action="switch-tab" data-tab="servers">
...
// --- Event delegation (replaces inline onclick/onchange so we can tighten CSP) ---
document.body.addEventListener('click', (e) => {
  const el = e.target.closest('[data-action]');
  if (!el) return;
  const action = el.dataset.action;
  switch (action) {
    case 'kill':              return killSession(el.dataset.id, el.dataset.name);
    case 'relaunch':          return relaunchSession(el.dataset.cwd, el.dataset.perm);
    case 'switch-tab':        return switchTab(el.dataset.tab);
    case 'toggle-settings':   return toggleSettings();
    case 'settings-backdrop': if (e.target === el) toggleSettings(); return;
    case 'copy-url':          return copyToClipboard(el.dataset.url);
    case 'refresh-sessions':  return refreshWithIndicator('refresh-sessions', refreshSessions);
    case 'refresh-servers':   return refreshWithIndicator('refresh-servers', refreshDevServers);
    case 'logout':            stopAutoRefresh(); localStorage.removeItem('launcher_token'); location.reload(); return;
...
document.body.addEventListener('change', (e) => {
  if (e.target && e.target.id === 'theme-select') applyTheme(e.target.value);
});
