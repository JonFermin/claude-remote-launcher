...
async function copyToClipboard(text) {
  // Try modern clipboard API first, then fall back to execCommand for mobile/HTTP contexts
  try {
  ...
  container.innerHTML = servers.map(s => {
    ...
    return `<div class="dev-card">
      ...
      <div class="dev-card-actions">
        <button class="btn-ghost" onclick="copyToClipboard('${safeUrl}')">${ICONS.clipboard} Copy</button>
        <a class="btn-primary" href="${safeUrl}" target="_blank" rel="noopener noreferrer">${ICONS.external} Open</a>
        ${expoBtn}
        <button class="btn-ghost" onclick="killDevServer(${s.port},'${escapeAttr(s.name)}')" style="color:var(--color-danger);border-color:var(--color-danger)">Kill</button>
      </div>
    </div>`;
  }).join('');
}

function killDevServer(port, name) {
  showConfirm('Kill server?', 'Kill ' + (name || 'server') + ' on port ' + port + '?', async () => {
    try {
      await api('DELETE', '/dev-server/' + port);
      showToast('Server on port ' + port + ' killed');
      await refreshDevServers();
    } catch {
      showToast('Failed to kill server');
    }
  });
}
