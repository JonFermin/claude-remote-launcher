...
// Also prunes stopped/error sessions older than 24h to prevent unbounded memory growth
...
export function startHealthSweep() {
  return setInterval(() => {
    ...
    for (const [id, session] of sessions) {
      ...
      // Prune old dead sessions
      if ((session.status === "stopped" || session.status === "error") && session.startedAt) {
